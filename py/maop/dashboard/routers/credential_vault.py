...
CredentialVault singleton + CRUD/rotate/audit operations live in
``maop.dashboard.services.credential_service``.  This router only does
request parsing, permission checks, service calls, response formatting,
and error handling.
"""
...
# ── Service layer ──────────────────────────────────────────────────
from maop.dashboard.services import credential_service
...
# ── Test-injection helper (backward compat — delegates to service) ──
def _set_vault(vault: Any) -> None:
    """供测试注入自定义 vault（隔离 DB）。委托到 credential_service。"""
    credential_service.set_vault(vault)
...
async def api_list_credentials(request: Request) -> dict[str, Any]:
    ...
    summaries = credential_service.list_credentials()
    return {
        "status": "ok",
...
async def api_get_credential(credential_id: str, request: Request) -> dict[str, Any]:
    ...
    require_admin(request)
    try:
        cred = credential_service.get_credential(credential_id, actor="api")
    except KeyError:
        raise HTTPException(404, f"Credential not found: {credential_id}")
    return {"status": "ok", "credential": cred}
...
async def api_create_credential(
    ...
    require_admin(request)
    try:
        cred_id = credential_service.create_credential(
...
async def api_delete_credential(credential_id: str, request: Request) -> dict[str, Any]:
    """删除凭证。"""
    require_admin(request)
    deleted = credential_service.delete_credential(credential_id, actor="api")
    if not deleted:
        raise HTTPException(404, f"Credential not found: {credential_id}")
...
async def api_rotate_credential(credential_id: str, request: Request) -> dict[str, Any]:
    ...
    require_admin(request)
    try:
        new_id = credential_service.rotate_credential(credential_id, actor="api")
    except KeyError:
        raise HTTPException(404, f"Credential not found: {credential_id}")
...
async def api_credential_audit(
    ...
    """查询凭证操作审计日志（按时间倒序）。"""
    require_admin(request)
    logs = credential_service.get_credential_audit(credential_id=credential_id, limit=limit)
