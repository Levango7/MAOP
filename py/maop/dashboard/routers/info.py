...
ARCHITECTURE = {
    ...
    "decisions": [
        ...
        ["熔断器", "三态(关闭/打开/半开) + SQLite持久化 + 降级链"],
        ["消息队列", "SQLite持久化 + 消费组 + 延迟队列 + 幂等消费"],
        ["向量搜索", "纯Python实现 + 余弦相似度 + sentence-transformers可选"],
        ["认证授权", "JWT + RBAC + API Key + 会话管理"],
        ["配置管理", "YAML + Pydantic校验 + 热重载 + 多环境覆盖"],
        ["监控体系", "Counter/Gauge/Histogram + Prometheus + 时序降采样"],
