"""GDPRComplianceManager — GDPR 数据主体权利、DPA、处理记录.
...
class GDPRComplianceManager:
    ...
    .. note::
       **接线状态**：本类曾长期无调用点（自 ``e2e0f2a`` 模块化拆分后从未
       实例化、无测试覆盖）。2026-09-21 起由
       ``maop/dashboard/routers/compliance.py`` 的 GDPR 端点接入，
       此前在用的 ``ComplianceManager`` 仅有 ``delete_user_data`` /
       ``export_user_data`` 两个方法，本类提供的下列能力实为缺失：
