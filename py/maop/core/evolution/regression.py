...
class RegressionTestRunner:
    ...
    async def run_suite(self, tests: list[TestCase], *, dispatcher: Any = None) -> list[TestResult]:
        # Parallelize test execution via asyncio.gather (§4.4).
        # Each run_test uses its own local TestResult — no shared mutable
        # state between tests. dispatcher.dispatch is async and runs on the
        # same event loop, so gather yields concurrent (not parallel) I/O
        # without race conditions. Result order matches input order.
        results = await asyncio.gather(
            *(self.run_test(t, dispatcher=dispatcher) for t in tests)
        )
        return list(results)
