...
def _rule_based_semantic_analyze(task: str, sub_tasks: list[SubTask]) -> tuple[list[SubTask], DependencyDAG, int]:
    ...
    # P4-§4.9: 维护 edges_set 作为边的倒排索引 (O(1) membership test)，
    # 避免 `(other.id, st.id) not in edges` 的 O(edges) 线性查找。
    # n = len(sub_tasks) 受 LoopConfig.max_subtasks=20 限制，但
    # dep_matches × n × n × edges 的最坏情况可达 ~800K 次比较，
    # 用 set 后降为 ~2K 次 (n² × m)。
    ...
    if len(sub_tasks) > 1:
        ...
        for i in range(len(sub_tasks) - 1):
            e = (sub_tasks[i].id, sub_tasks[i + 1].id)
            edges.append(e)
            edges_set.add(e)

    # Check for explicit dependency overrides
    # P4-§4.9: 预计算 description.lower() 避免在内层循环中重复调用 .lower()。
    desc_lower = {st.id: st.description.lower() for st in sub_tasks}
    for st in sub_tasks:
        for dep_hint in dep_matches:
            dep_hint_lower = dep_hint.strip().lower()
            for other in sub_tasks:
                if other.id != st.id and dep_hint_lower in desc_lower[other.id] and (other.id, st.id) not in edges_set:
                    e = (other.id, st.id)
                    edges.append(e)
                    edges_set.add(e)
