...
Usage::
    ...
    # P1-§4.5: tool calls are independent — execute concurrently.
    # asyncio.gather preserves input order, so results align with calls.
    results = await asyncio.gather(*[bridge.execute(call) for call in calls])
