...
最终确认需要修复的真实问题 4 个，截至 2026-08-22（commit 0ff95bb）**全部已修复**（P0-1、P0-2、P0-3、P1-4）。
...
### 4.1 P0-1：AGENT/DAG 步骤有 executor 时不检查返回结果错误（已修复）
...
### 4.4 P1-4：依赖失败时 on_failure 默认空字符串导致继续执行（已修复）
