...
## 7. 1 小时中间验证结果（评审条件项补测）

为修复评审条件项「soak 测试仅 15 min 而非 48 h」，在 15 min 冒烟测试基础上补测 1 h（3600 s）中间验证，作为通往 48 h 完整长稳的阶梯门禁。

表：1h 测试参数

| 参数 | 值 | 说明 |
|------|-----|------|
| `--duration` | 3600 s（1 h） | 中间验证时长 |
| `--sample-interval` | 30 s | 采样间隔 |
| `--mem-rps` | 15 | 记忆读写次数/秒 |
| `--dag-interval` | 5 s | DAG 执行间隔 |
| `--leak-threshold` | 1.0 MB/h | 内存泄漏告警阈值 |
...
| 实测记忆吞吐 | ≈ 9.62 ops/s |
| 实测 DAG 吞吐 | ≈ 0.18 DAG/s（约每 5.5 s 一次，符合设定） |
| 泄漏判定资格 | 是（`leak_eligible=true`，时长 ≥ 1 h） |

### 7.2 四项指标汇总

表：1h 四项指标汇总（min/max/avg/最终值/趋势斜率）

| 指标 | 单位 | min | max | avg | 最终值 | 趋势斜率 | 泄漏疑似 |
|------|------|-----|-----|-----|--------|----------|----------|
| 内存 RSS | bytes | 13 971 456 | 51 159 040 | 34 815 153 | 13 971 456 | -12.3618 MB/h | 否 |
| 文件句柄数 | count | 150 | 158 | 151.84 | 155 | -1.0860 handles/h | 否 |
| 连接池占用 | count | 0 | 0 | 0 | 0 | 0.0000 conns/h | 否 |
| CPU 使用率 | % | 3.10 | 28.10 | 16.61 | 25.00 | 2.9869 %/h | 否 |

说明：

- **内存 RSS**：1 h 内从 48.79 MB 高点回落到 13.32 MB，斜率 -12.36 MB/h（负增长）。因测试时长 ≥ 1 h 已具备泄漏判定资格，斜率为负且远低于 1.0 MB/h 阈值，**无泄漏**。RSS 高点出现在预热阶段（前若干采样），随后 GC 稳定在 30-32 MB 区间，末尾负载线程退出时 RSS 回收至 13.32 MB。
- **文件句柄数**：稳定在 150-158 之间，斜率 -1.086 handles/h（负），无泄漏。
- **连接池占用**：始终为 0，与 15 min 测试一致，符合预期。
- **CPU 使用率**：3.1%-28.1% 波动，平均 16.61%，斜率 2.99 %/h（轻微上升但在合理波动范围内），无异常。

### 7.3 指标曲线图（ASCII sparkline）

```
内存 RSS (MB):
  min=13.32  max=48.79  final=13.32
  ▇▇▇▇▇▇▇█▇▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▁
文件句柄数:
  min=150  max=158  final=155
  ▆▆█▃▃▂▂█▂▂▂▂▂▂▂█▁▂▂▂▂▂█▂▂▂▂▂▂▃▂▁▂█▂▂▅▂▂▂▂▂▅▂███▂▂▂▂▂▂▂▂▂▂▂▁█
连接池占用:
  min=0  max=0  final=0
  ────────────────────────────────────────────────────────────
CPU 使用率 (%):
  min=3.10  max=28.10  final=25.00
  ▅▄▅▂▅▇▄▄▅▄▄▃▄▃▅▃▃▂▄▄▃▃▄▃▅▄▆▇▃▁▃█▄▃▄▃▄▅▆▆▄▄▄▂▆▄▇▄▅▄▄▅▆▄▆▄▄▅▆▇
```

### 7.4 1h 结论

**PASS** — 1 小时中间验证中四项指标均平稳，`overall_pass=true`，无泄漏迹象。

判定依据（时长 ≥ 1 h，已启用泄漏判定）：

- 内存 RSS 斜率 -12.36 MB/h，远低于 1.0 MB/h 阈值，负增长 → 无泄漏
- 文件句柄数斜率 -1.086 handles/h，低于 10 handles/h 阈值 → 无泄漏
- 连接池占用斜率 0 conns/h → 无泄漏
- CPU 使用率斜率 2.99 %/h，在合理波动范围 → 无异常

1 h 中间验证补强了 15 min 冒烟测试的置信度：在具备泄漏判定资格的时长下，四项指标趋势斜率均为负或极小正值，证明 MAOP 个人版在持续 1 h 负载下内存/句柄/连接池稳定，无长稳隐患。48 h 完整长稳测试仍建议用户自行运行以获得最终交付门禁结论。

输出文件：

- `deliverables/soak-test-data.csv`：116 个采样点原始数据（1h 运行后覆盖）
- `deliverables/soak-test-summary.json`：1h 汇总统计 + 趋势分析
- `deliverables/soak-1h-output.txt`：1h 运行完整日志