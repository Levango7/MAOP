...
- **覆盖率 80% → 85%**：补齐 `py/tests/` 关键路径用例，CI 阈值同步上调。（⚠️ 更正 2026-08-14：实际基线约 18.5%，85% 为历史目标未达成，见 `py/scripts/check_coverage_ratchet.py` 的 `FLOOR=18.0`）
...
- [ ] CI 覆盖率阈值 ≥ 85%：⚠️ 未达成，实际约 18.5%（ratchet 基线 `FLOOR=18.0`），原"实测达标"记录不实，已更正。
